...
- **선택된 투어 명칭 말줄임표 제거 및 줄바꿈 허용 (`StepTechnicalTour.tsx`, `StepSummary.tsx`)**:
  - 투어 선택 단계 우측 사이드바 및 등록 요약 화면 내 선택된 투어 명칭(Technical Tour)에 적용되어 있던 `truncate` 클래스를 제거하여, 텍스트가 가로폭을 초과하더라도 말줄임표(`...`)로 잘리지 않고 정상적으로 다음 줄로 줄바꿈되어 전체가 보이도록 개선.
- **등록 확인 화면 내 선택된 항목 명칭 폰트 두께 일반화 (`StepSummary.tsx`)**:
  - 최종 확인 화면(Summary)에서 선택된 패키지 요율 명칭(Registration Category)과 선택된 소셜 프로그램 명칭(Social Programme)에 지정되어 있던 `font-semibold` 클래스를 제거하여, 일반 굵기의 폰트로 렌더링되도록 통일성 있게 일괄 조정.
