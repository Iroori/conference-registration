# IABSE INCHEON 2026 — 개발 지침서 (GEMINI.md)

이 파일은 Gemini(Google AI)가 이 프로젝트에서 세션을 시작할 때 **자동으로 읽히는 규칙서**입니다.
...
**IABSE INCHEON 2026 Annual Conference** 참가 등록 및 결제 시스템.
...
| 주요 도메인 | `user`, `iasbse`, `option`, `payment`, `config`, `dev` |
...
### Infrastructure
- **AWS Lightsail** (Ubuntu 22.04, ap-northeast-2 서울)
- **SQL Server 2022 Express** (prod)
...
- `@StrongPassword`는 SHA-256 해시에 적용 불가 → **프론트엔드에서만 강도 검증**
...
### 9-1. 브랜치 전략
- `main` — 배포 기준 브랜치 (push 시 자동 AWS 배포)
- 기능 단위로 브랜치를 분리하고 PR을 통해 main에 병합

### 9-2. 커밋 메시지 형식 (Conventional Commits)
...
### 9-3. 배포 주의사항
- main 브랜치에 push/merge → GitHub Actions가 자동으로 빌드 + AWS Lightsail 배포
- 배포 파이프라인: Maven 빌드 → Vite 빌드 → SCP/rsync → systemctl restart → 헬스체크
- **main에 잘못된 코드가 올라가면 즉시 운영 서버에 반영됨** — 반드시 테스트 후 push
...
| 날짜 | 파일 | 주요 내용 |
|------|------|----------|
| 2026-03-31 | registration-payment-redesign.md | 회원가입 개선 + 결제 플로우 전면 재설계 + 영어 UI 전환 + 비밀번호 SHA-256 |
| 2026-04-16 | aws-deployment.md | AWS Lightsail 인프라 구성 + MSSQL + SES + GitHub Actions CI/CD |
